...
public class SchedulerController : Controller
{
    ...
    public async Task<IActionResult> Index(int? fieldId, int? semId, string gW, string gC, string gPs, string gP, string gL, string gJ, string gS, string gWf)
    {
        try
        {
            ...
            var hasGroupFilters = !string.IsNullOrEmpty(gW) ||
                                  !string.IsNullOrEmpty(gC) || !string.IsNullOrEmpty(gL) || 
        ...
        {
            ViewBag.Fields = new List<FieldOfStudy>();
            ViewBag.Semesters = new List<Semester>();

            return View(new SchedulerViewModel
            {
                Lessons = new List<TimetableEntryDto>(),
